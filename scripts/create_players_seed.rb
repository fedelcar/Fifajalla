...
while page_number < 344
	puts "Processing page #{page_number}"
	current_page = Nokogiri::XML(open("http://www.futhead.com/15/career-mode/players/?page=#{page_number}"))
	current_page.encoding = 'ISO-8859-1'
	...
	player_links.each do |link|
		player_number += 1
		player_page = Nokogiri::XML(open("http://www.futhead.com#{link}"))
		player_page.encoding = 'ISO-8859-1'
		...
		#puts "Player ##{player_number}: #{last_name}. overall: #{overall}   age: #{age}  league: #{league}  "
		...
		if primary_position == "GK" 
			...
			if(first_name == "")
				...
				creation_strings.push("Player.create(id:#{player_number}, last_name: \"#{last_name}\", overall: #{overall}, league: \"#{league}\", primary_position: \"#{primary_position}\", secondary_position: \"#{secondary_position}\", games_played: 0, goals: 0, assists: 0, own_goals: 0, yellow_cards: 0, red_cards: 0, diving:#{diving}, handling: #{handling}, kicking: #{kicking}, positioning: #{positioning}, reflexes: #{reflexes}, club: \"#{club}\", nation: \"#{nation}\", age: #{age}, height: \"#{height}\", attack_WR: \"#{attack_WR}\", defend_WR: \"#{defend_WR}\", weak_foot: #{weak_foot}, skill_moves: #{skill_moves})")
				puts "Player ##{player_number}: #{last_name}. Done."
			else
				creation_strings.push("Player.create(id:#{player_number}, first_name: \"#{first_name}\", last_name: \"#{last_name}\", overall: #{overall}, league: \"#{league}\", primary_position: \"#{primary_position}\", secondary_position: \"#{secondary_position}\", games_played: 0, goals: 0, assists: 0, own_goals: 0, yellow_cards: 0, red_cards: 0, diving:#{diving}, handling: #{handling}, kicking: #{kicking}, positioning: #{positioning}, reflexes: #{reflexes}, club: \"#{club}\", nation: \"#{nation}\", age: #{age}, height: \"#{height}\", attack_WR: \"#{attack_WR}\", defend_WR: \"#{defend_WR}\", weak_foot: #{weak_foot}, skill_moves: #{skill_moves})")
